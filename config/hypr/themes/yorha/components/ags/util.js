...
async function get_cursor() {
  const res = await Hyprland.messageAsync("cursorpos");
...
const activePopup = Variable(null, {});
let activePopupCloser = null;

globalThis.dark = dark;

function registerActivePopup(closer, popup = null) {
  activePopupCloser = closer;
  activePopup.setValue(popup);
}

function clearActivePopup(closer = null) {
  if (!closer || activePopupCloser === closer) {
    activePopupCloser = null;
    activePopup.setValue(null);
  }
}

function hasActivePopup() {
  return activePopupCloser !== null;
}
...
function closeActivePopup() {
  ...
  const closer = activePopupCloser;
  activePopupCloser = null;
  activePopup.setValue(null);
  closer();
  return true;
