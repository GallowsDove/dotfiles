...
const popup_overlay = Box({
  ...
  connections: [
    [
      activePopup,
      (self) => {
        self.children = activePopup.value ? [activePopup.value] : [];
      },
    ],
  ],
});

const settings_info_overlay = Box({
  hpack: "start",
  vpack: "end",
  hexpand: true,
  vexpand: true,
  classNames: ["settings-info-anchor"],
  child: Info({useAssetsDir:parentAssetsDir,parentDir:parentConfigDir}),
});
  ...
  panes_container = Box({
    ...
    setup: (self) =>
      Utils.timeout(1, () => {
        ...
        execAsync(`agsv1 -b bg_settings -q`).catch(() => null).finally(() => {
          Utils.timeout(10, () => {
            execAsync(`agsv1 -b bg_settings -c ${parentConfigDir}/windows/settingsbg/settingsbg.js`).catch(print)
          })
        });
        ...
        (self, windowName, visible) => {
          if (windowName ==  "settings") {
            ...
            if (!visible) {
              ...
              print("closingbg")
              execAsync(`agsv1 -b bg_settings -r closeBgSettings()`).catch(print).then(print)
              self.classNames = arrremove(self.classNames, "opening");
              self.classNames = arradd(self.classNames, "closing");
            } else {
              print("openingbg")
              execAsync(`agsv1 -b bg_settings -r openBgSettings()`).catch(print)
