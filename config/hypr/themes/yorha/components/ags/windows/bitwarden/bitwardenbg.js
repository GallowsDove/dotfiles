...
const getDirectionalRatios = ({
  ...
  const leftScore = (leftPoint[0] - centerPointX) * directionX + (leftPoint[1] - centerPointY) * directionY;
  const rightScore = (rightPoint[0] - centerPointX) * directionX + (rightPoint[1] - centerPointY) * directionY;
  const yScore = (yPoint[0] - centerPointX) * directionX + (yPoint[1] - centerPointY) * directionY;
  ...
  const animate = async ({ opening, currentAnimationId }) => {
    ...
    while (true) {
      ...
      for (let i = 0; i < rows * cols; i += 1) {
        ...
        const dist = distanceFromCenter(centerPointX, centerPointY, screenCenterX, screenCenterY);
